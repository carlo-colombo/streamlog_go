...
var _ = Describe("BeVisible", func() {
	...
	It("should match when element is visible", func() {
		...
		Expect(locator).Should(playwrightmatchers.BeVisible(1))
	})

	It("should not match when element is hidden", func() {
		// Set up a hidden element
		err := page.SetContent(`
		...
		Expect(locator).ShouldNot(playwrightmatchers.BeVisible(1))
	})
	...
	It("should match when element becomes visible", func() {
		// Set up an element that's initially hidden
		err := page.SetContent(`
	...
	It("should fail with appropriate message when element is not visible", func() {
		...
		success, err := playwrightmatchers.BeVisible().Match(locator)
		Expect(err).Should(BeNil())
